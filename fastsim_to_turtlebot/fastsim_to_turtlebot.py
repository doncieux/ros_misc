...
class TranslateFastsimTurtlebot():
    def __init__(self):
        ...
        self.sb = rospy.Subscriber("/mobile_base/evts/bumper", BumperEvent, self.Bumper_callback)
  ...
  # sleep just makes sure TurtleBot receives the stop command prior to shutting down the script
    ...
    def Speed_right_callback(self,data):
        ...
        rospy.loginfo("Received speed right: "+str(self.speed_right))

    def Bumper_callback(self,data):
        rospy.loginfo("Received bumper")
        if (data.bumper==data.LEFT):
            self.left_bumper=data.state==data.PRESSED
        elif (data.bumper==data.RIGHT):
            self.right_bumper=data.state==data.PRESSED
